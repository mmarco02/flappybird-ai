...
train_policy_iterations = 90
train_value_iterations = 90
lam = 0.97
target_kl = 0.01
hidden_sizes = (64, 64)
...
#load weights of the policy if they exist
if os.path.exists("saved/ppo_flappy_bird.keras"):
    actor = tf.keras.models.load_model("saved/ppo_flappy_bird.keras")
    print("Loaded saved policy")
    print(f"Model weights: {actor.get_weights()}")
    ...
    for epoch in range(epochs):
        ...
        for t in range(steps_per_epoch):
            ...
            if terminal or (t == steps_per_epoch - 1):
                print(f"Episode {t} Return: {episode_return}. Episode Length: {episode_length}")
        ...
        eval_rewards.append(sum_return / num_episodes)

        # Save weights of the policy
        policy_saver = tf.keras.models.save_model(actor, "saved/ppo_flappy_bird.keras")
        print(f"Policy saved to saved/policy")
