...
if os.path.exists("saved/dqn_flappy_bird.keras"):
    dqn_agent = tf.keras.models.load_model("saved/dqn_flappy_bird.keras")
    print("Loaded saved model")
    print(f"Model weights: {dqn_agent.get_weights()}")
else:
    dqn_agent = DQN(num_actions)
    print("Created new model")

dqn_agent.compile(optimizer=tf.keras.optimizers.Adam(learning_rate=learning_rate),
...
for episode in range(num_episodes):
    ...
    if episode % log_interval == 0:
        print(f"Episode={episode}, loss={loss.numpy()}, exploration_prob={exploration_prob}")

    if episode % save_interval == 0:
        dqn_agent.save("saved/dqn_flappy_bird.keras")
        print("Saved model")
