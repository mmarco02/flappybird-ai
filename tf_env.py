...
class FlappyBirdEnvironment(tf_py_environment.py_environment.PyEnvironment):
    ...
    def _step(self, action):
        ...
        self._reward = self._score + (self._distance / 200.0)
    ...
    def get_observation(self):
        bird_y = float(self._flappy_bird_game.get_bird_y())
        bird_velocity = float(self._flappy_bird_game.get_bird_velocity())
        distance_to_next_pipe = float(self._flappy_bird_game.get_distance_to_next_pipe())
        vertical_distance_to_next_top_pipe = float(self._flappy_bird_game.get_vertical_distance_to_next_top_pipe())
        vertical_distance_to_next_bottom_pipe = float(
            self._flappy_bird_game.get_vertical_distance_to_next_bottom_pipe())
